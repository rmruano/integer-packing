...
class IntegerQuartet extends IntegerBase {
    ...
    public function __construct($integer16bitA, $integer16bitB, $integer16bitC, $integer16bitD) {
        ...
        foreach (array("A","B","C","D") as $intNum) {
            ...
            if (${"integer16bit".$intNum} > static::MAX_16BIT_INTEGER_VALUE || ${"integer16bit".$intNum} < -(static::MAX_16BIT_INTEGER_VALUE+1)) {
                throw new OutOfBoundsException("Integer".$intNum." exceeds the 16bit range [".-(static::MAX_16BIT_INTEGER_VALUE+1)." to " .
